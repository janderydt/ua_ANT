...
dsn(dsn==-32767) = nan;
dsn = flipdim(permute(dsn,[3 1 2]),3);
%dsn_err_scale = 0.001;
dsn_err = ncread(ncfilen,'height_change_rmse');
dsn_err(dsn_err==-32767) = nan;
dsn_err = flipdim(permute(dsn_err,[3 1 2]),3);
% calculate elevation change ds with respect to targettime
...
dsp = flipdim(permute(dsp,[3 1 2]),3);
dsp_err = ncread(ncfilep,'height_change_err');
dsp_err = flipdim(permute(repmat(dsp_err,1,1,size(dsp,1)),[3 1 2]),3);
% calculate elevation change ds with respect to targettime
